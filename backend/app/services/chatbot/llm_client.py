...
def generate_response(prompt: str, system_instruction: str = None) -> str:
    ...
    """
    logger.info(f"Generating LLM response using provider '{settings.LLM_PROVIDER}', model '{settings.LLM_MODEL_NAME}'")
    ...
    try:
        if settings.LLM_PROVIDER == "gemini":
            ...
            if "v1" in settings.LLM_API_ENDPOINT:
                url = f"{settings.LLM_API_ENDPOINT.rstrip('/')}/chat/completions"
                messages = []
                if system_instruction:
                ...
                url = f"{settings.LLM_API_ENDPOINT.rstrip('/')}/api/chat"
                messages = []
                if system_instruction:
                    messages.append({"role": "system", "content": system_instruction})
                messages.append({"role": "user", "content": prompt})
...
def ai_data_recovery(raw_ocr_text: str) -> dict:
    ...
    try:
        ...
        # Case type lock and deterministic precedence logic
